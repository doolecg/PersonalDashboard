...
function mergeProbability(values) {
    const valid = values.filter((entry) => typeof entry.value === "number");
    if (!valid.length)
        return undefined;
...
function valuesForTime(sources, picker, weights) {
    return sources.map((source) => ({ value: picker(source), weight: weights[source.source] ?? 0 }));
}
function mergePoint(points, fallbackTime) {
    return {
...
function mergeDailyPoint(points, fallbackDate) {
    return {
...
export function mergeWeatherSources(sources) {
    if (!sources.length) {
