...
function numberAt(record, key, index) {
...
function stringAt(record, key, index) {
    const value = record?.[key];
    if (Array.isArray(value)) {
        const item = typeof index === "number" ? value[index] : undefined;
...
function toVisibilityKm(value) {
    return typeof value === "number" ? roundToSingleDecimal(value / 1000) : undefined;
}
function toWeatherPoint(record, time, index) {
    return {
        time,
...
export function normalizeOpenMeteo(raw, source, city) {
    const hourlyTimes = Array.isArray(raw.hourly?.time) ? raw.hourly.time.filter((time) => typeof time === "string") : [];
    const dailyTimes = Array.isArray(raw.daily?.time) ? raw.daily.time.filter((time) => typeof time === "string") : [];
    return {
        source,
