...
// next load.
const readItems = (ctx, name) => withSyncTracking(() => getStorageDriver().list(ctx.userId, name));
const writeItems = (ctx, name, items) => withSyncTracking(() => getStorageDriver().replaceCollection(ctx.userId, name, items));
...
function findMatch(items, args, fields) {
...
export const todosTool = {
    ...
    execute: async (args, ctx) => {
        ...
        const items = await readItems(ctx, "todos");
        if (action === "add") {
            const text = str(args.text);
            if (!text)
                return { error: "No to-do text provided." };
            const todo = { id: randomUUID(), text, done: false, createdAt: new Date().toISOString() };
        ...
        if (action === "complete" || action === "remove") {
            ...
            if (!target)
                return { error: "Couldn't find a matching to-do." };
...
export const notesTool = {
    ...
    execute: async (args, ctx) => {
        const action = str(args.action) || "list";
        const items = await readItems(ctx, "notes");
        if (action === "add") {
            const text = str(args.text);
            const title = str(args.title);
            if (!text && !title)
                return { error: "No note text provided." };
            const note = { id: randomUUID(), text, title: title || undefined, updatedAt: new Date().toISOString() };
        ...
        if (action === "remove") {
            ...
            if (!target)
                return { error: "Couldn't find a matching note." };
...
export const remindersTool = {
    ...
    execute: async (args, ctx) => {
        const action = str(args.action) || "list";
        const items = await readItems(ctx, "reminders");
        if (action === "add") {
            const text = str(args.text);
            if (!text)
            ...
            const reminder = { id: randomUUID(), text, due, done: false };
        ...
        if (action === "complete" || action === "remove") {
            const target = findMatch(items, args, ["text"]);
            if (!target)
                return { error: "Couldn't find a matching reminder." };
            const next = action === "complete"
...
export const calendarTool = {
    ...
    description: "List, add, or remove events on the user's calendar (the upcoming/calendar cards). Use ISO 8601 datetimes for start and end.",
    parameters: {
        type: "object",
        properties: {
            action: { type: "string", enum: ["list", "add", "remove"], description: "What to do." },
    ...
    execute: async (args, ctx) => {
        const action = str(args.action) || "list";
        const items = await readItems(ctx, "events");
        if (action === "add") {
            const title = str(args.title);
