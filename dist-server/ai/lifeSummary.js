...
// generic line.
function composeFallback(events, weather, tasks, reminders, advice) {
    // A single, data-rich paragraph describing the user's day.
    const today = [];
    if (weather)
        today.push(`It's ${weather} today${advice ? `, so ${advice}` : ""}.`);
    else if (advice)
        today.push(`Today, ${advice}.`);
    if (events.length) {
        const next = events[0];
        const when = formatWhen(next.start);
        today.push(`You have ${events.length} event${events.length > 1 ? "s" : ""} on, next is ${next.title}${when ? ` at ${when}` : ""}.`);
    }
    if (tasks.length) {
    ...
    if (!today.length)
        return "Nothing on your plate right now, and a clear day ahead.";
    return today.join(" ");
}
...
function cacheKey(events, weather, tasks, reminders, news, advice) {
...
export function generateLifeSummary(events, weather, tasks = [], reminders = [], news = [], advice) {
...
async function buildLifeSummary(events, weather, tasks = [], reminders = [], news = [], advice) {
    ...
    try {
        ...
        if (message)
            return { summary: message, source: "ai" };
    }
    catch {
