...
import { dismissSuggestion, generateSuggestions, markAccepted } from "../assistant/suggestions.js";
...
async function suggestionsFor(userId, force = false) {
...
assistantSuggestionsRouter.post("/actions/execute", async (req, res, next) => {
    try {
        ...
        if (action === "create_todo") {
            ...
            const items = await withSyncTracking(() => driver.list(userId, "todos"));
            await withSyncTracking(() => driver.replaceCollection(userId, "todos", [
                ...items,
            ...
            const items = await withSyncTracking(() => driver.list(userId, "reminders"));
            await withSyncTracking(() => driver.replaceCollection(userId, "reminders", [...items, { id: randomUUID(), text, due, done: false }]));
        }
        else if (action === "create_note") {
            const text = str(payload.text);
            if (!text)
                return res.status(400).json({ message: "Note text is required." });
            const items = await withSyncTracking(() => driver.list(userId, "notes"));
            await withSyncTracking(() => driver.replaceCollection(userId, "notes", [
                ...items,
        ...
        if (body.suggestionId) {
            if (action === "dismiss")
                await dismissSuggestion(userId, body.suggestionId);
            else
                await markAccepted(userId, body.suggestionId);
            cache.delete(userId);
        }
        res.json({ ok: true });
    }
