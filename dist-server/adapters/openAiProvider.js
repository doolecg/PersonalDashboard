...
const openAiErrorMessage = (status) => {
    if (status === 401)
        return "OpenAI authentication failed. Check OPENAI_API_KEY.";
    if (status === 402)
        return "OpenAI billing is unavailable.";
    if (status === 429)
...
async function openAiResponse(request, model) {
    if (!env.openAiApiKey)
...
export function createOpenAiProvider(model = env.openAiModel) {
    return {
        name: "openai",
