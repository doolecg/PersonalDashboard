...
export const LOCAL_USER = { userId: "local", email: "local@dev", mode: "local" };
...
function cacheToken(token, auth) {
...
export async function verifyAccessToken(token) {
    ...
    const auth = { userId: data.user.id, email, mode: "supabase" };
...
export function getAuth(req) {
    if (!req.auth)
        throw new Error("Route is missing auth middleware");
    return req.auth;
}
