...
function geminiRole(role: AiMessage["role"]) {
  return role === "assistant" ? "model" : "user";
}

function geminiBody(request: AiRequest) {
...
const errorMessage = (status: number) => {
  if (status === 401 || status === 403) return "Gemini authentication failed. Check GEMINI_API_KEY.";
...
export function createGeminiProvider(model = env.geminiModel): AiProvider {
  return {
    name: "gemini",
    async complete(request) {
      ...
      try {
        ...
        if (error instanceof DOMException && error.name === "AbortError") throw new Error("Gemini request timed out.");
        throw error;
      } finally {
