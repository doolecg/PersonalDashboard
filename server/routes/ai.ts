...
aiRouter.patch("/status", (req, res) => {
  const body = (req.body ?? {}) as { mode?: unknown; model?: unknown };
  ...
  setAiSelection({
    mode: hasMode ? (body.mode as Parameters<typeof setAiSelection>[0]["mode"]) : undefined,
    model: hasModel ? (body.model as string | null) : undefined
  });
  res.json(getAiStatus());
});
...
function assistantMessagesFromBody(body: unknown): AssistantMessage[] {
  ...
  return value.messages
    ...
    .map((message) => ({ role: message.role as AssistantMessage["role"], content: message.content as string }));
}

aiRouter.post("/assistant", async (req, res) => {
  const messages = assistantMessagesFromBody(req.body);
  if (!messages.length) return res.status(400).json({ message: "At least one message is required." });
